  ...
  <div class="flex-1 flex flex-col min-h-0 relative overflow-hidden bg-studio-bg select-none">
    ...
    <div class="flex-1 overflow-y-auto min-h-0 p-3 sm:p-4">
      ...
      <div v-if="loading && !files.length" class="space-y-2">
        <div v-for="i in 8" :key="i" class="h-11 rounded-lg bg-studio-surface border border-white/5 animate-pulse-subtle flex items-center px-3 gap-3">
          <div class="w-6 h-6 rounded bg-white/10"></div>
          <div class="h-3 rounded bg-white/10 w-1/3"></div>
          <div class="ml-auto h-3 rounded bg-white/10 w-16"></div>
        </div>
      </div>
      ...
      <div v-else-if="viewMode === 'list'" class="space-y-1">
        <div
          v-for="file in files"
          :key="file.Key"
          class="group flex items-center gap-3 px-3 py-2 rounded-lg border transition-all cursor-pointer"
          :class="[
            selectedKey === file.Key
              ...
              : selectedKeys.has(file.Key || '')
                ...
                : 'bg-studio-surface/60 border-white/5 hover:border-white/15 hover:bg-studio-surface text-zinc-300'
          ]"
        ...
        >
          ...
          <!-- Metadata -->
          <div class="flex items-center gap-4 font-mono text-[11px] text-zinc-500 shrink-0">
            <span class="w-16 text-right">{{ formatBytes(file.Size ?? 0) }}</span>
            <span class="hidden sm:inline-block w-20 text-right text-zinc-600">{{ formatDate(file.LastModified) }}</span>
          </div>
